...
const fileRestaurant = require('fs').createWriteStream('cassandradb/restaurant.txt');
// use the line below if you want to generate a smaller test file
// const fileRestaurant = require('fs').createWriteStream('cassandradb/restaurantShort.txt');
...
const generateRestaurants = async () => {
  ...
  for (let id = 1; id <= 1e7; id += 1) {
    ...
    if (!fileRestaurant.write(`${id}|${JSON.stringify(dishes)}|${link}\n`)) {
